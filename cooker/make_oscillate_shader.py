#!/usr/bin/env python
"""Build a getTime()-driven ARBITRARY-AXIS OSCILLATION (sway/wobble) shader from a stock V203 RENDSHAD — the
GENERAL V79 node-rotation port for clips that DON'T spin continuously but swing out and back (Dragon Ball /
Snake Way's King Kai planet sways 0deg->73deg->0deg per clip; (1-cos) shape, ZERO velocity at t=0). Sibling of
make_rotate_shader.py (continuous spin); same getTime() toolchain as wispscale / flipbook.

The forward VERTEX stage rotates inPos about a fixed axis by a TIME-VARYING angle:
  a(t) = (AMP/2) * (1 - cos(2*pi*time/PERIOD))     # one-sided sway, peaks at AMP at the clip midpoint
  v'   = v*cos(a) + (axis x v)*sin(a) + axis*(axis.v)*(1-cos(a))   # Rodrigues
AMP is SIGNED (preserves which way it swings first); axis + AMP + PERIOD baked as constants. The cooker
generates one shader per (amp,axis,period,blend) ON DEMAND — no per-env pre-baked shaders.

Usage: python make_oscillate_shader.py <AMP_rad_signed> <PERIOD_s> <AX> <AY> <AZ> <SRC.bin> <OUT.bin>
REPACK identical to make_rotate_shader.py (append grown vertex module at EOF, repoint the stage uoffset).
"""
import struct, sys, math

AMP    = float(sys.argv[1]) if len(sys.argv) > 1 else math.radians(73.0)
PERIOD = float(sys.argv[2]) if len(sys.argv) > 2 else 33.333
AX     = float(sys.argv[3]) if len(sys.argv) > 3 else 0.0
AY     = float(sys.argv[4]) if len(sys.argv) > 4 else 1.0
AZ     = float(sys.argv[5]) if len(sys.argv) > 5 else 0.0
SRC    = sys.argv[6] if len(sys.argv) > 6 else "cooker/nuxd_unlit_shader.bin"
OUT    = sys.argv[7] if len(sys.argv) > 7 else "cooker/osc_gen.surface.bin"
_l = math.sqrt(AX*AX+AY*AY+AZ*AZ) or 1.0
AX, AY, AZ = AX/_l, AY/_l, AZ/_l
W = 2*math.pi/PERIOD if PERIOD else 0.0
...
def main():
    ...
    c_w=fconst(W); c_halfamp=fconst(AMP*0.5); c_one=fconst(1.0)
    ...
    pt=nid(); t=nid(); arg=nid(); carg=nid(); omcarg=nid(); a=nid(); cs=nid(); sn=nid(); omc=nid()
    dotav=nid(); crossv=nid(); term1=nid(); term2=nid(); k=nid(); term3=nid(); tmp=nid(); rot=nid()
    body=[
        [65,[0,pUf,pt,gu,c_time]],
        [61,[0,tFloat,t,pt]],
        [133,[0,tFloat,arg,t,c_w]],                  # arg = time*W      (W=2pi/period)
        [12,[0,tFloat,carg,glsl,GLSL_COS,arg]],      # cos(arg)
        [131,[0,tFloat,omcarg,c_one,carg]],          # 1-cos(arg)
        [133,[0,tFloat,a,omcarg,c_halfamp]],         # a = (1-cos(arg))*(AMP/2)  -> 0..AMP sway (signed AMP = direction)
        [12,[0,tFloat,cs,glsl,GLSL_COS,a]],          # cos(a)
        [12,[0,tFloat,sn,glsl,GLSL_SIN,a]],          # sin(a)
        [131,[0,tFloat,omc,c_one,cs]],               # 1-cos(a)
    ]
    ...
    body += [
        [148,[0,tFloat,dotav,axisVec,load_res]],            # dot(axis,v)
        [12,[0,tV3,crossv,glsl,GLSL_CROSS,axisVec,load_res]], # cross(axis,v)
        [142,[0,tV3,term1,load_res,cs]],                    # v*cos(a)
        [142,[0,tV3,term2,crossv,sn]],                      # cross*sin(a)
        [133,[0,tFloat,k,dotav,omc]],                       # (axis.v)*(1-cos(a))
    ...
    print("wrote %s (amp=%.4f rad period=%.1fs axis=(%.3f,%.3f,%.3f) bound %d)"%(OUT,AMP,PERIOD,AX,AY,AZ,bound))
    return 0
